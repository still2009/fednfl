...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None, tb_writer=None, cid=0, bid=0):
        ...
        try:
            for trial in range(self.config['restarts']):
                x_trial, labels = self._run_trial(x[trial], input_data, labels, dryrun=dryrun, tb_writer=tb_writer, cid=cid, bid=bid)
                # Finalize
    ...
    def _init_images(self, img_shape):
        ...
        elif '.dlg.init' in self.config['init']:
            print('load fixed specified init!!')
            return torch.load(self.config['init']).to(self.setup['device'])
        else:
            raise ValueError()

    def _run_trial(self, x_trial, input_data, labels, dryrun=False, tb_writer=None, cid=0, bid=0):
        ...
        try:
            for iteration in range(max_iterations):
                ...
                rec_loss, tv_loss = optimizer.step(closure)
                if self.config['lr_decay']:
                    scheduler.step()
                ...
                with torch.no_grad():
                    ...
                    if (iteration + 1 == max_iterations) or iteration % 10 == 0:
                        if tb_writer != None:
                            tb_writer.add_scalar(f'C{cid}/dlg_B{bid}_rec_loss', rec_loss, iteration)
                            tb_writer.add_scalar(f'C{cid}/dlg_B{bid}_tv_loss', tv_loss, iteration)
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, label):
        ...
        def closure():
            ...
            raw_rec_loss = reconstruction_costs([gradient], input_gradient,
                                            cost_fn=self.config['cost_fn'], indices=self.config['indices'],
                                            weights=self.config['weights'])
            tv_loss = 0
            if self.config['total_variation'] > 0:
                tv_loss += self.config['total_variation'] * TV(x_trial)
            
                rec_loss = raw_rec_loss + tv_loss
            rec_loss.backward()
            if self.config['signed']:
                x_trial.grad.sign_()
            return raw_rec_loss, tv_loss
        return closure
...
class FedAvgReconstructor(GradientReconstructor):
    ...
    def _gradient_closure(self, optimizer, x_trial, input_parameters, labels):
        def closure():
            ...
            raw_rec_loss = reconstruction_costs([parameters], input_parameters,
                                            cost_fn=self.config['cost_fn'], indices=self.config['indices'],
                                            weights=self.config['weights'])
            tv_loss = 0
            if self.config['total_variation'] > 0:
